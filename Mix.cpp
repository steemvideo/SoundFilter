...
TsampleFormat TmixerMatrix::calc_matrix(TsampleFormat &infmt,TsampleFormat &outfmt,int pos)
{
	outfmt.setChannels(pos==16?infmt.nchannels:chConfigs[pos].nchannels,pos==16?infmt.channelmask:chConfigs[pos].channelmask);
	outfmt.dolby=pos==16?infmt.dolby:chConfigs[pos].dolby;
	...
	double clev=0.8;             // central mix level
	double slev=2.0;             // surround mix level
	double lfelev=0.5;         // lfe mix level
	...
	else
	{
		...
		bool expand_stereo_allowed =  !in_nrear;
		bool voice_control_allowed =  (in_nfront == 2 || in_nfront == 1);
